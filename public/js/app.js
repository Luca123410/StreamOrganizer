...
const { createApp, ref, computed, onMounted, onBeforeUnmount, watch, nextTick } = Vue
    ...
    const app = createApp({
    	setup() {
            ...
            const pendingImportNames = ref([]); // <-- Aggiunta per Importazione Migliorata
 		...
 		// --- Functions ---
            ...
            // ▼▼▼ MODIFICA: Import Confirmation Step 4 ▼▼▼
        	const closeImportConfirm = () => { 
                showImportConfirm.value = false; 
                pendingImportData.value = null; 
                importSource.value = ''; 
                pendingImportNames.value = []; // Pulisci i nomi
            };

        	const confirmImport = () => {
        	 	try {
        	 	 	// I dati sono GIÀ analizzati (non serve più JSON.parse)
            ...
            // ▲▲▲ FINE: Import Confirmation Step 4 ▲▲▲
            ...
            const handleFileImport = (event) => {
        	 	...
        	 	reader.onload = e => {
        	 	 	try {
        	 	 	 	...
        	 	 	 	const importedData = JSON.parse(jsonString); // Analizza subito
        	 	 	 	if (!Array.isArray(importedData)) throw new Error("Dati JSON non validi.");
        	 	 	 	
        	 	 	 	pendingImportData.value = importedData; // Salva l'oggetto analizzato
        	 	 	 	pendingImportNames.value = importedData.map(a => a?.manifest?.name || 'Addon Sconosciuto'); // Estrai i nomi
        	...
        	const copyShareLink = () => {
         	...
         	const checkAllAddonsStatus = async () => {
        	 	...
        	 	// Usiamo Promise.allSettled per processarli tutti
        	 	await Promise.allSettled(addons.value.map(async (addon) => {
        	 	 	addon.status = 'checking';
        	 	 	addon.errorDetails = null;
        	 	 	
        	 	 	try {
        	 	 	 	// ▼▼▼ INIZIO MODIFICA (Step 2) ▼▼▼
        	 	 	 	// Chiamiamo il nostro backend invece del fetch diretto
        	 	 	 	...
        	 	 	 	if (!response.ok) {
        	 	 	 	 	// Errore del nostro server (500, 504, ecc.)
        	 	 	 	...
        	 	 	 	if (data.status === 'ok') {
        	 	 	 	 	...
        	 	 	 	 	// L'addon è offline o ha restituito un errore
        	 	 	 	 	throw new Error(data.details || 'Check failed');
        	 	 	 	}
        	 	 	 	// ▲▲▲ FINE MODIFICA (Step 2) ▲▲▲
        	 	 	 	...
        	 	 	 	errorCountLocal++;
        	 	 	 	// Non rilanciamo l'errore, così Promise.allSettled continua
        	 	 	}
        	 	}));
            ...
            // ▼▼▼ FUNZIONE GITHUB (MIGLIORATA) (Step 3) ▼▼▼
            const fetchGithubInfo = async (addon) => {
                ...
                // Regex per trovare un link GitHub (es. https://github.com/owner/repo)
                const githubRepoRegex = /(https?:\/\/github\.com\/[\w-]+\/[\w-]+)/;
                
                // Regex per trovare un link GitHub Pages (es. https://owner.github.io/repo)
                ...
                // 1. Cerca un link GitHub diretto nella descrizione
                match = description.match(githubRepoRegex);
                if (match) {
                    repoUrl = match[0];
                }

                // 2. Se non trovato, cerca un link GitHub diretto nell'URL del manifesto
                if (!repoUrl) {
                ...
                // 3. Se non trovato, cerca un link GitHub Pages nell'URL del manifesto
                if (!repoUrl) {
                    match = transportUrl.match(githubPagesRegex);
                    if (match) {
                        // Ricostruisci l'URL del repo (es. da https://owner.github.io/repo -> https://github.com/owner/repo)
                ...
                // Se non abbiamo trovato NESSUN link, esci
                if (!repoUrl) {
                ...
                try {
                    ...
                    // Imposta un errore "silenzioso" per non mostrare il loader all'infinito
                    addon.githubInfo = { error: err.message }; 
                } finally {
                    addon.isLoadingGithub = false;
                }
            };
            // ▲▲▲ FINE FUNZIONE GITHUB ▲▲▲
            
             // ▼▼▼ FUNZIONE toggleAddonDetails CORRETTA (Step 3) ▼▼▼
             const toggleAddonDetails = (addon) => { 
        	 	addon.isExpanded = !addon.isExpanded;
        	 	
        	 	// Se stiamo aprendo, avvia il caricamento
        	 	if (addon.isExpanded) {
        	 	 	fetchGithubInfo(addon);
        	 	}
       	 };
             // ▲▲▲ FINE toggleAddonDetails ▲▲▲
  	...
  	// Non resettare l'email, potrebbe servire in entrambe le modalità
        	...
        	const dismissWelcomeScreen = () => {
        	 	...
        	 	if (importedConfigFromUrl.value) {
        	 	 	const importedData = importedConfigFromUrl.value; // Dati già analizzati
            ...
            animation: 150,  // Riduci per meno lag su mobile
            ghostClass: "ghost-class",
            handle: ".drag-handle",
            forceFallback: true,
            scrollSensitivity: 150,  // Aumenta per scroll durante drag
            bubbleScroll: true,
            delay: isMobile.value ? 400 : 300,  // Più delay su mobile per distinguere da scroll
            delayOnTouchOnly: true,
            touchStartThreshold: 15,  // Aumenta per tollerare piccoli movimenti (evita drag accidentali)
            fallbackTolerance: 5,  // Tolleranza per drag imprecise su touch
            filter: '.no-drag'  // Aggiungi classe .no-drag su elementi non draggabili (es. bottoni dentro addon)
         }));
